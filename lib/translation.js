...
export function buildSystemPrompt(sourceLang, targetLang) {
  ...
  return [
    ...
    '2. Keep the tone and register: a casual message between neighbours stays casual, a formal notice stays formal, polite forms stay polite, humour and irony stay.',
    '3. Keep exactly as written: names of people, companies and the community; place, street and building names; block, apartment, garage and storage numbers; amounts and currencies; dates and times; phone numbers, e-mail addresses, URLs, @mentions, hashtags and emojis.',
    '4. Spanish official and community terms (for example Junta General, Junta Directiva, administrador de fincas, presidente, derrama, IBI, Ayuntamiento): use the usual equivalent in the target language; if there is no clear equivalent, keep the Spanish term.',
    ...
    `7. ${src} is the author's app language; if a text is actually written in another language, translate from that language. If a text is already in ${tgt}, return it unchanged. If a text mixes languages, translate every part into ${tgt}. An empty text stays empty.`,
