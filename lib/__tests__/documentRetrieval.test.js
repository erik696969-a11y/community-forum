...
describe('retrieveRelevantDocumentChunks', () => {
  ...
  it('breaks score ties deterministically by chunk_index', () => {
    ...
    const result = retrieveRelevantDocumentChunks(chunks, 'site manager position');
    expect(result.map((c) => c.id)).toEqual(['earlier', 'later']);
  });

  it('uses the exported minimum meaningful-token threshold', () => {
    expect(MIN_MEANINGFUL_TOKENS).toBe(2);
  });
});

describe('formatDocumentExcerptsForPrompt', () => {
  it('returns a stable empty marker when no chunks are supplied', () => {
    expect(formatDocumentExcerptsForPrompt([])).toBe('(none)');
    expect(formatDocumentExcerptsForPrompt(null)).toBe('(none)');
  });

  it('formats document and chunk titles together with the excerpt', () => {
    const result = formatDocumentExcerptsForPrompt([
      chunk({
        document_title: 'AGM Minutes 2026',
        chunk_title: 'Item 4: Budget',
        chunk_text: 'Approved budget text.',
      }),
    ]);
    expect(result).toBe('[AGM Minutes 2026 - Item 4: Budget]\nApproved budget text.');
  });

  it('separates multiple excerpts and supports a missing chunk title', () => {
    const result = formatDocumentExcerptsForPrompt([
      chunk({
        document_title: 'AGM Minutes 2026',
        chunk_title: null,
        chunk_text: 'First excerpt.',
      }),
      chunk({
        document_title: 'Community Statutes',
        chunk_title: 'Parking',
        chunk_text: 'Second excerpt.',
      }),
    ]);
    expect(result).toBe(
      '[AGM Minutes 2026]\nFirst excerpt.\n\n[Community Statutes - Parking]\nSecond excerpt.'
    );
  });
});
