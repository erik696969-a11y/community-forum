...
const GENERIC_TERMS = new Set([
  ...
  // Second confirmed false-positive source, same day: "found" is a
  // near-universal verb for reporting ANY incident ("I found a leak",
  // "I found dog poo", "I found broken glass") and carries almost no
  // topic-specific signal on its own - "I found a dog poo before my
  // door. What should I do?" matched SEC-02 (burglary discovered)
  // purely because "found" and "door" both happened to appear in
  // SEC-02's own example queries, with zero genuine burglary content in
  // the question. "door" is deliberately NOT added here - unlike
  // "found", it carries real signal for genuinely door-related
  // scenarios (BLD-04, SEC-08, BLD-06) and filtering it globally would
  // weaken those legitimate matches.
  'found',
  'encontre', 'encontré',
  'trouve', 'trouvé',
  'gefunden',
  // "someone" was investigated the same day (appears in
  // example_user_queries of 18 different intent codes - by far the
  // broadest overlap found) and CONFIRMED in production to pollute the
  // secondary "attached" scenario list (e.g. a correctly-answered
  // missing-parcel question also attached ADM-01/MED-06/SEC-02/SEC-06
  // purely via "someone" overlap). However, unlike "very"/"people"/
  // "found", removing it caused two REAL regressions in the canonical
  // 240-question suite, including one safety-relevant one: "Someone
  // damaged a communal door" (deliberate vandalism, SEC-05) started
  // losing to COM-08 (routine maintenance defect) once "someone" no
  // longer contributed to SEC-05's score margin over COM-08's own,
  // separately-legitimate "damaged"/"door" overlap. A compensating
  // SEC-01 singleton for "breaking" was tried and also reverted - it
  // fixed the intended case but then wrongly fired on "A resident keeps
  // breaking community rules" (nothing to do with a break-in). Given a
  // confirmed-safe fix isn't available yet, "someone" is deliberately
  // NOT added here - the attached-list pollution it causes is real but,
  // based on the one confirmed production case, does not corrupt the
  // primary/visible answer, which is a meaningfully smaller harm than
  // the regressions its removal introduced. Revisit only with a
  // narrower, scenario-specific approach (e.g. strengthening the
  // individual colliding entries' own vocabulary, as done for NUI-01
  // and NUI-03 above) rather than a blanket GENERIC_TERMS entry.
...
const HIGH_CONFIDENCE_SINGLETONS = {
  ...
  'SEC-01': [
    ...
    // "in the process of", not a completed/historical action. Bare
    // English "breaking" was tried here too (to compensate for removing
    // "someone" from GENERIC_TERMS) but reverted - "breaking" alone is
    // too ambiguous ("breaking community rules", "breaking news") and
    // wrongly fired on non-burglary questions. Since "someone" itself
    // was also reverted (see GENERIC_TERMS), no compensating singleton
    // is needed here.
    'robando', 'entrando', 'einbrechen', 'bricht', 'effraction',
    // Confirmed gap from live multilingual testing: "effraction" alone
    // did not cover phrasings using "cambrioler"/"cambriolage" instead
    // (e.g. "Quelqu'un est en train de cambrioler mon appartement
    // maintenant" matched nothing). Both are unambiguous burglary-only
    // words in French with no other common meaning. Note: the tokenizer
    // extracts the INFINITIVE form "cambrioler" from natural phrasing
    // like "être en train de cambrioler" - the conjugated form
    // "cambriole" added first did NOT match this and was a real
    // (caught-before-shipping) near-miss; both forms are kept since
    // either could appear depending on phrasing.
    'cambriolage', 'cambriole', 'cambrioler',
  ],
...
// Fallback for when the model produces JSON containing an unescaped
// literal double-quote inside a string value (e.g. answer text quoting
// a button label like `press "0"`) - this is invalid JSON and
// safeParseJson() above correctly returns null for it, but the
// resident should still see a clean answer rather than the raw,
// broken JSON text (confirmed in production: this exact failure mode
// surfaced the raw ```json {...} block directly in the chat).
//
// Since the response schema has fixed, known field names in a fixed
// order, each field can be recovered with a non-greedy regex anchored
// on the NEXT field's key rather than on the first quote character -
// this correctly spans embedded, unescaped quotes inside the value,
// because the literal key name of the next field (e.g. "urgency")
// essentially never appears naturally inside a resident-facing answer.
// Returns null if even the safety-critical "answer" field cannot be
// recovered this way, so the caller's existing null-handling still
// applies as the final fallback.
export function repairTruncatedJson(raw) {
  if (!raw) return null;
  const text = raw.trim().replace(/^```(json)?/i, '').replace(/```$/, '').trim();

  const answerMatch = text.match(/"answer"\s*:\s*"([\s\S]*?)"\s*,\s*"urgency"/);
  if (!answerMatch) return null;

  const unescape = (s) => s.replace(/\\"/g, '"').replace(/\\n/g, '\n').replace(/\\\\/g, '\\');
  const result = { answer: unescape(answerMatch[1]) };

  const urgencyMatch = text.match(/"urgency"\s*:\s*"(yellow|orange|red)"/);
  if (urgencyMatch) result.urgency = urgencyMatch[1];

  // call112 is safety-critical: only set it when a clean true/false
  // token is actually found, so a genuinely broken response still
  // falls through to the caller's own safe emergency-detection
  // fallback rather than silently defaulting to false.
  const call112Match = text.match(/"call112"\s*:\s*(true|false)/);
  if (call112Match) result.call112 = call112Match[1] === 'true';

  const sourceStatusMatch = text.match(/"source_status"\s*:\s*"([a-z_]+)"/);
  if (sourceStatusMatch) result.source_status = sourceStatusMatch[1];

  const clarifyingMatch = text.match(/"clarifying_question"\s*:\s*"([\s\S]*?)"\s*\}/);
  result.clarifying_question = clarifyingMatch ? unescape(clarifyingMatch[1]) : '';
