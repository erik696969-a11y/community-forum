...
function tokenizeForPhraseMatch(text) {
  return (text || '')
    ...
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
    .filter((w) => (w.length >= 3 && !STOPWORDS.has(w)) || STATE_MARKERS.has(w));
}
...
function scoreEntry(entry, questionTokens, questionPhraseTokens) {
  ...
  const singletonMatched = singletonAnchors.some((word) => questionTokens.has(word));

  const eligible = phraseMatched || singletonMatched || meaningfulMatches.size >= MIN_MEANINGFUL_TOKENS;
...
export function retrieveRelevantEntries(entries, question) {
  ...
  const scored = entries.map((entry) => ({ entry, ...scoreEntry(entry, questionTokens, questionPhraseTokens) }));
