...
export function resolvePlaceholdersInText(text, contacts, config) {
  ...
  return text.replace(/\[([A-Z0-9_]+)\]/g, (match, token) => {
    ...
    if (contactSpec) {
      ...
      return value || UNRESOLVED_PLACEHOLDER_TEXT;
    }
...
// The example_user_queries in logic_json are full natural phrasings ("Water
// is coming through my ceiling") which are a richer retrieval signal than
// the short intent_tags alone, especially for non-English phrasing patterns
// residents actually type. Merge both into one keyword pool per entry.
export function keywordPoolForEntry(entry) {
  const tagWords = entry.keywords || [];
  const queryWords = (entry.logic_json?.example_user_queries || []).flatMap((q) => tokenize(q));
  return [...tagWords, ...queryWords];
}
