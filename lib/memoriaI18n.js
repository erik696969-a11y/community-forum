...
import { moreDict } from './memoriaI18nMore';
...
const dict = {
  ...
  urgentUnbudgeted: {
    en: 'Urgent unbudgeted expense (Statutes art. XXX (30): approved by President + Administrator, to be ratified by the Board)',
    es: 'Gasto urgente no presupuestado (Estatutos art. XXX (30): autorizado por Presidente + Administrador, a ratificar por la Junta)',
    fr: 'Dépense urgente hors budget (statuts art. XXX (30) : autorisée par Président + Administrateur, à ratifier par le conseil)',
    de: 'Dringende außerplanmäßige Ausgabe (Satzung Art. XXX (30): von Präsident + Verwalter genehmigt, vom Vorstand zu ratifizieren)',
  },
...
export function mt(lang, key, vars) {
  const entry = dict[key] || govDict[key] || opsDict[key] || importDict[key] || budgetDict[key] || moreDict[key];
