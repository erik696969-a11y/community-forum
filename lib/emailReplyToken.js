...
export async function resolveReplyToken(adminClient, address) {
  ...
  const { data } = await adminClient
    ...
    .select('post_id, user_id, expires_at, revoked_at')
    .eq('token', token)
    .maybeSingle();

  if (!data) return null;

  // Reject expired or manually revoked tokens - they stay in the table for
  // audit purposes, but must no longer be usable to post as the member.
