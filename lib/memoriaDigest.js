...
export function collectDigestItems(data, today, { includeDemo = false } = {}) {
  ...
  for (const c of (data.cases || []).filter(keep)) {
    if (!['open', 'in_progress', 'waiting'].includes(c.status) || !c.next_step_due) continue;
    if (c.next_step_due <= soonLimit) items.push({ kind: 'case', name: c.next_step ? `${c.title} — ${c.next_step}` : c.title, date: c.next_step_due, isDemo: Boolean(c.is_demo) });
  }
